...
{
	{RGB(0,0,150),RGB(0,0,100),RGB(255,0,0)},//цвет: окна, дисплея, шрифта
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	(
		NULL, //exStyle
		g_sz_WINDOW_CLASS,//Clas
		g_sz_WINDOW_CLASS,//Title
		//WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX,//Style
	...
	if (hwnd == NULL)
	{
		MessageBox(NULL, "WindowCreation falied", NULL, MB_OK | MB_ICONERROR);
		return 0;
	}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		HBITMAP hbmpZero =(HBITMAP) LoadImage(GetModuleHandle(NULL), "Zero.bmp", IMAGE_BITMAP, g_i_DOUBLE_BUTTON_SIZE, g_i_BUTTON_SIZE,LR_LOADFROMFILE);
		...
		{
			HDC hdc = (HDC)wParam;//С сообщением WM_CTLCOLOREDIT в 'wParam' принимается HDC элемента  EditContol
	...
	case WM_COMMAND:
	{
		...
		static BOOL input_operation = FALSE;	//Срабатывает на знак операции
		...
		if (LOWORD(wParam) >= IDC_BUTTON_0 && LOWORD(wParam) <= IDC_BUTTON_9)
		{
			...
			if (strcmp(sz_buffer, "0") == 0)strcpy(sz_buffer, sz_digit);//string copy strcpy(dst,src)
			else strcat_s(sz_buffer, sz_digit);//string concatenation(dst,src) dst destination строка получатель, src  source источник dst+=src
		...
		if (LOWORD(wParam) == IDC_BUTTON_POINT)
		{
			...
			if (strchr(sz_buffer, '.')==0)strcat_s(sz_buffer, ".");
			SendMessage(hEdit, WM_SETTEXT, 0, (LPARAM)sz_buffer);
			input = TRUE;
		}
